...
function parseManifest(raw) {
  const items = raw.split('\n').map((line) => line.trim()).filter((line) => line && !line.startsWith('#')).map((line, index) => {
    ...
    if (!/^goals\/[A-Za-z0-9._/-]+$/.test(branch) || branch.includes('..')) fail(`manifest line ${index + 1} has unsafe branch: ${branch}`);
  ...
  const maxBatch = Math.min(Number.parseInt(process.env.MAX_BATCH ?? '3', 10), 4);
  if (!Number.isInteger(maxBatch) || maxBatch < 1) fail('MAX_BATCH must be an integer from 1 to 4');
...
if (reviewers.length !== 2) fail('REVIEWERS must name exactly two T0 council agents');
if (new Set(reviewers).size !== reviewers.length) fail('T0 reviewers must be distinct');
...
if (strictBatch && items.every((item) => !item.failure)) {
  strictIntegrationBranch = `goals/strict-integration-${process.pid}-${Date.now()}`;
